...
const WorkCard = (props) => {
  ...
  const renderWorkItems = () => {
    ...
    props.workTask.workItems.forEach( (workItem, index) => {
      ...
      if (!index) {
        frag = <Table.Row >
                <Table.Cell textAlign='center' rowSpan={props.workTask.workItems.length}>
                  {props.workTask.workTask}
                  <div style={{float: "right"}} hidden={props.hidden}>
                    <Button
                      floated="left"
                      icon="edit"
                      size="mini"
                      onClick={() => props.editWorkTask(props.workTask)}/>
                  </div>
                </Table.Cell>
